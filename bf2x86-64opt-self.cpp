...
std::string bf_to_x86_64(const std::string& src_bf_code, bool opt_sl, bool opt_scan) {
    ...
    std::string assembly_code = R"(    .global main
    .global bf_main
    .extern malloc
    .extern memset
    .extern free
...
main:
    push %rbx
    mov $50016, %edi           # size = tape_size + 16
    call malloc
    test %rax, %rax
    je malloc_failed
    mov %rax, %rbx             # Save original pointer for free
    add $15, %rax
    and $-16, %rax             # Align to 16 bytes
    mov %rax, %rdi             # dest = aligned tape pointer
    xor %esi, %esi             # value = 0
    mov $50000, %edx           # size = tape_size
    call memset
    mov %rax, %rdi             # bf_main(tape)
    call bf_main
    mov %rbx, %rdi             # free(original pointer)
    call free
    pop %rbx
    xor %eax, %eax
    ret
...
bf_main:
    ...
    push %r14
    push %r15
    mov %rdi, %r15            # Save the tape pointer in %r15
